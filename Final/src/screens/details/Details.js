...
class Details extends Component {
    ...
    componentWillMount = async () => {
        ...
        await fetch(this.props.baseUrl + "movies/" + this.props.match.params.id,
            {
                method: "GET",
                headers: {
    ...
    /*Handler methods for showing artists and movie rating*/
    ...
    ratingHandler = (id) => {
        let starIconList = [];
        for (let star of this.state.ratings) {
            let starNode = star;
            if (star.id <= id) {
        ...
        this.setState({ ratings: starIconList });
    }
    ...
    render() {
        ...
        return (
            <div className="details">
                ...
                <div className="movieDetailsContainer-flex">
                    ...
                    <div className="ratingArtists">
                        <Typography> <span className="bold">Rate this movie: </span></Typography>

                        {this.state.ratings.map(star => (
                            <StarBorderIcon className={star.color} key={"star" + star.id} onClick={() => this.ratingHandler(star.id)} />
                        ))}
                        ...
                        <GridList cellHeight={160} cols={2}>
                            {movie.artists.map(artist => (
                                <GridListTile className="gridTile" onClick={() => this.artistsHandler(artist.wiki_url)} key={artist.id}>
